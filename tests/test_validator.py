...
class TestValidateIntentAdvanced:
    ...
    def test_invalid_join_type(self, sample_schema):
        """Test that invalid join types are silently normalized to INNER (not rejected).
        
        The validator's normalize_joins() converts unknown join types to INNER
        without raising an error, for forward-compatibility with new join keywords.
        """
        intent = {
            "metrics": [{"metric":"count","aggregation":"COUNT","target_column":"order_id"}],
            "fact_table": "fact_orders",
        ...
        # INVALID join type is silently normalized to INNER — validation passes
        ok, errors = validate_intent(intent, sample_schema)
        assert ok, f"Validation failed unexpectedly: {errors}"
